...
int actualLength = 40;
int output;
// range of value to be use in fuzzy logic
int len[] = {10, 50, 100};
...
void currentlyCutting(){
  ...
  for(int i = 0; i < wireQuantity; i++){
    ...
    delay(500);
    output = (int)(wireLength - actualLength)/mmPerStep;
    // Compare the measured length with the desired length
    // using fuzzy logic
    if ((actualLength >= len[0] && actualLength < len[1]) && (wireLength >= len[1] && wireLength <= len[2])){
      Serial.println("clockwise");
      myStepper.step(output);
      delay(500);     
    }
    else if ((actualLength >= len[1] && actualLength < len[2]) && (wireLength == len[2])){
      Serial.println("clockwise");
      myStepper.step(output);
      delay(500);     
    }
    else if ((actualLength >= len[1] && actualLength <= len[2]) && (wireLength >= len[0] && wireLength < len[1])){
      Serial.println("counterclockwise");
      myStepper.step(output);
      delay(500);     
    }
    else if ((actualLength > len[1] && actualLength <= len[2]) && (wireLength == len[1])){
      Serial.println("counterclockwise");
      myStepper.step(output);
      delay(500);     
    }
    else if ((actualLength > len[0] && actualLength <= len[2]) && (wireLength == len[0])){
      Serial.println("counterclockwise");
      myStepper.step(output);
      delay(500);     
    }
